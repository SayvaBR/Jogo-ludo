...
// The previous save format used mismatched entrances and must not be silently relocated.
...
function Dice({value,rolling,disabled,onClick}: {value:number;rolling:boolean;disabled:boolean;onClick:()=>void}) {
  return <button className={`dice ${rolling?'dice--rolling':''}`} onClick={onClick} disabled={disabled} aria-label={rolling?'Dado rolando':value?`Dado ${value}. Lançar dado`:'Lançar dado'}>
    <span className="dice-face">{value === 0 ? <b className="dice-placeholder" aria-hidden="true">?</b> : dots[value].map(([x,y],i)=><i key={i} style={{gridColumn:x+1,gridRow:y+1}} />)}</span>
  </button>;
}
export default function App() {
  ...
  const start=(count:number,versusCpu:boolean)=>{
    ...
    lock.current=false;setBusy(false);setRolling(false);setDiceFace(0);setAnimation(null);setGame(next);setPage('game');setModal(false);
  };
  const rollDie=useCallback(async()=>{
    ...
    // Draw only once per turn; animation, CPU and human rolls share identical odds.
    const value=fairDie();
    if(!settings.reduced) await pause(420);
    const next=roll(game,value);
    setDiceFace(value);setGame(next);setRolling(false);setBusy(false);lock.current=false;
